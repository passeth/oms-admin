...
| :--- | :--- | :--- | :--- | :--- |
| **`cm_raw_order_lines`** | 주문 원장 | 업로드된 모든 주문 내역. (트리거가 항시 대기 중) | 매일 아침 (업로드) | `site_order_no`, `option_text`, `matched_kit_id` |
| **`cm_order_gifts`** | 사은품 지급 내역 | 배송 담당자가 확정한 사은품 리스트. | 오후 (사은품 지정 시) | `order_line_id`, `gift_kit_id`, `gift_qty` |
...
3. **Action:**
    ...
    * [50명 일괄 적용] 버튼 클릭.
    * -> `cm_order_gifts` 테이블에 50건 생성됨.

### [Step 4] 최종 다운로드

1. **화면:** "출고 센터".
2. **Action:** [오늘자 배송리스트 다운로드] 버튼 클릭.
3. **결과:** 택배사 양식에 딱 맞는 `result.csv` 다운로드.
