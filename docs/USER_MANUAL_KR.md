...
**최신 배포 결과 및 출고 현황 (`Dispatch Dashboard`)**
> "오늘 출고해야 할 물건이 총 몇 개지?"

- **메뉴:** [출고 관리] -> [출고 현황 (Dispatch)]
- **기준:** **오늘 업로드(Upload Date)**된 주문 중, 처리가 완료(**DONE**)된 주문을 집계합니다.
- **기능:**
  - **Picking List:** 창고에서 물건 찾을 때 쓰는 합계 리스트.
  - **Ecount Excel:** 이카운트에 업로드할 출고 전표 파일.
- **주의:** 사은품도 하나의 '주문'으로 생성되어 이곳에 포함됩니다.
...
**Q. 출고 리스트에 사은품이 안 보여요!**
A. **사은품 키트의 BOM(구성품)**이 등록되어 있는지 확인해주세요.
사은품도 시스템 입장에서는 '상품(Kit)'입니다. 이 사은품이 어떤 단품으로 구성되어 있는지(예: 박스 1개 + 본품 1개) `BOM 관리` 메뉴에서 등록되어 있어야 수량 집계가 가능합니다. 구성품 정보가 없으면 수량이 0이 되어 리스트에서 제외될 수 있습니다.
