...
export function SiteTrendTable({ data }: { data: { months: string[], rows: any[] } }) {
    ...
    return (
        <Card className="col-span-12">
            <CardHeader>
                <CardTitle>Site Performance Matrix (6 Months)</CardTitle>
            </CardHeader>
            <CardContent>
                            ...
                            {data.rows.map((row, i) => (
                                <tr key={i} className="border-b last:border-0 hover:bg-muted/50 transition-colors">
                                    <td className="py-2 px-3 font-medium">
                                        <Link
                                            href={`/sales/site/${encodeURIComponent(row.site)}`}
